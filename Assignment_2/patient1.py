...
class Patient1:
    ...
    def connect_to_doctor(self, host="127.0.0.1", port=65432):
        print(f"[Patient1] Connecting to doctor with host:port {host}:{port}")
        self.sock= socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.connect((host, port))

        data= self.sock.recv(4096)
        doc_info= json.loads(data.decode())
        print("[Patient1] doc info -", doc_info)
        self.p= doc_info["p"]
        self.g= doc_info["g"]
        ...
        print(f"[Patient1] x_private={self.x_private}, y_public={self.y_public}")

        auth_req= self.build_auth_req()
        print("[Patient1] sending auth_req with TSi, ephemeral")
    ...
    def build_auth_req(self):
        ...
        print(f"[Patient1] TSi={TSi_val}, ephemeral={self.ephemeral}.")
        time.sleep(2) 
    ...
    def process_doc_response(self, resp):
        ...
        print(f"[Patient1] ephemeral decrypted - {dec_ephem}")
    ...
    def handle_incoming_msg(self, msg_dict):
        ...
        if opcode==30:
            print("[Patient1] Received group key. Decrypting with SK.")
            enc_GK_hex= msg_dict["encrypted_GK"]
            enc_GK= bytes.fromhex(enc_GK_hex)
            try:
                GK_bytes= self.aes_decrypt_256(self.SK, enc_GK)
                self.GK= GK_bytes
                print(f"[Patient1]   GK - {self.GK.hex()}")
            except:
                print("[Patient1]   ERROR: decrypt GK fail.")
        elif opcode==40:
            if not self.GK:
                print("[Patient1] No GK can't decrypt broadcast.")
                return
            ...
            try:
                ...
                print(f"[Patient1] BROADCAST - '{plaintext}'")
            except:
                print("[Patient1] ERROR - can't decrypt broadcast.")
        else:
            print("[Patient1] unknown ", msg_dict)
    ...
    def chat_loop(self):
        print("[Patient1] You may type plaintext messages. 'bye' to end.")
        while True:
            msg= input("[Patient1] ")
            if not msg:
                msg= "(empty)"
